...
static void _Putfld(printf_struct *x, va_list *args, char type, char *buff)
{
	...
	case 'G':
		...
		x->buff = (char *)&buff[x->n0];
        x->value.f64 = ((f32)x->value.f64); // TODO: PRECOMP HACK: idk why but this fixes float printing
		_Ldtob(x, type);
		break;
	case 'n':
		if (x->length == 'h') {
...
int _Printf_patch(outfun prout, char *arg, const char *fmt, va_list args)
{
	printf_struct x;
	const char *s;
	char c;
